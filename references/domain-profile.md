# Domain Profile

<!-- Fill in this file for your field. It is read by the Librarian agent (lit-review)
     and the Explorer agent (data-finder) to focus their searches.
     Change any section to match your specific subfield or project. -->

## Field

Political Science — Duke University

---

## Top 5 Journals

<!-- Librarian 1 searches these first. List in priority order.
     Change these to match your subfield if needed. -->

- American Political Science Review — https://www.cambridge.org/core/journals/american-political-science-review
- American Journal of Political Science — https://onlinelibrary.wiley.com/journal/15405907
- Journal of Politics — https://www.journals.uchicago.edu/journals/jop/about
- Comparative Political Studies — https://journals.sagepub.com/home/cps
- World Politics — https://www.cambridge.org/core/journals/world-politics

## Secondary Journals

<!-- Librarian 2 searches these. Include subfield and adjacent journals.
     Swap in more specific outlets if your work is in a narrow subfield. -->

- Political Research Quarterly
- Political Behavior
- Legislative Studies Quarterly

---

## Working Paper Repositories

<!-- Pre-filled. Edit search URL patterns for your field's keywords. -->

- **NBER:** https://www.nber.org/papers — search: https://www.nber.org/search?q=[KEYWORDS]
- **SSRN:** https://www.ssrn.com — search: `site:ssrn.com [KEYWORDS]`
- **IZA:** https://www.iza.org/publications/dp — search: `site:iza.org dp [KEYWORDS]`
- **arXiv (if applicable):** https://arxiv.org/search/ — econ or stat sections

---

## Common Datasets

<!-- Explorer agent checks these first before doing a broader search.
     Add or remove datasets to match your research area. -->

<!-- Format: - [Dataset Name] — [Provider] — [Public/Restricted] — [URL] -->

- American National Election Studies (ANES) — Stanford / University of Michigan — Public — https://electionstudies.org
- Cooperative Election Study (CES) — Harvard — Public — https://cces.gov.harvard.edu
- Varieties of Democracy (V-Dem) — University of Gothenburg — Public — https://www.v-dem.net
- Correlates of War (COW) — University of Michigan — Public — https://correlatesofwar.org
- World Bank Development Indicators — World Bank — Public — https://databank.worldbank.org/source/world-development-indicators

---

## Key Researchers / Groups

<!-- Librarian uses these for author-based targeted searches.
     Replace or extend with researchers most relevant to your specific project. -->

<!-- Format: - [Name] — [Institution] — [Google Scholar or SSRN URL] -->

- John Aldrich — Duke University — https://scholars.duke.edu/person/aldrich
- Karen Remmer — Duke University — https://scholars.duke.edu/person/remmer
