...
- **Writes:** `references/domain-profile.md` (field, journals, datasets, colors), `CLAUDE.md` (project name/institution)
...
**Configurable:** Section 4 (Visual Identity) reads hex colors from `references/domain-profile.md`. Set via `/research-setup` or edit the Institutional Colors section of domain-profile directly.
...
Runs on every session start. Creates `quality_reports/{plans,session_logs,specs,merges}/`, `references/papers/`, and `manuscripts/` directories. Copies all files from the plugin's `templates/` and `references/` directories into your project (no-clobber — your edits are never overwritten). This is how `references/domain-profile.md` and the starter templates arrive in your project.
...
Edit the "Institutional Colors" section of `references/domain-profile.md` to set your institution's hex color palette. These colors are read by `rules/r-code-conventions.md` and applied to all ggplot2 figures generated by `/data-analysis`. Set automatically by `/research-setup`.
...
```
/research-setup ──writes──► references/domain-profile.md (field, journals, datasets, colors)
                             CLAUDE.md
