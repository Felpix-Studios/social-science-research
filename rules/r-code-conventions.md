...
### Figure Dimensions (format-dependent)
```r
# Beamer/slides:    width = 12, height = 5, bg = "transparent"
# Paper/manuscript: width = 6.5, height = 4   (standard column width)
# Web/HTML:         width = 8,   height = 5
ggsave(filepath, width = WIDTH, height = HEIGHT)
# Add bg = "transparent" only when saving for Beamer/slide output
```
...
|---------|--------|------------|
| Missing `bg = "transparent"` for slides | White boxes on Beamer slides | Add `bg = "transparent"` when saving for slide output |
